...
class ParserData
{
     ...
     *
     * Перезаписывает данные при нахождении пользователя с тем же ip.
     *
     ...
     *
     * Если в база будут найдены записи с аналогичным ip, датой и временем, то запись будет обновлена.
     *
     * Формат данных в файле:
    ...
    public function parsingDisplacements($absFilePath)
    {
        ...
        while ($string = fgets($file)) {
            ...
            if (!$user->getId()) {
                ...
                $this->em->persist($user);
                $this->relationCount++;
                // нужен тут, что бы не крашилась при попытке добавить пользователя несколько раз
                $this->em->flush();
            } else {
